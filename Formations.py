...
class Dastan:
    ...
    def PlayGame(self):
        GameOver = False
        PlayerOneFormation = int(input("""
        1. Trench
        2. Pincer
        3. Corner Kick
        4. Dagger
        5. Default for Both"""))
        if PlayerOneFormation == 1:
            self.FormationTrench(1)
        elif PlayerOneFormation == 2:
            self.FormationPincer(1)
        elif PlayerOneFormation == 3:
            self.FormationCornerKick(1)
        elif PlayerOneFormation == 4:
            self.FormationDagger(1)
        elif PlayerOneFormation == 5:
            self.__CreatePieces(self._NoOfPieces)
        #Fix option 5
        PlayerTwoFormation = int(input("""
        1. Trench
        2. Pincer
        3. Corner Kick
        4. Dagger"""))
        if PlayerTwoFormation == 1:
            self.FormationTrench(2)
        elif PlayerOneFormation == 2:
            self.FormationPincer(2)
        elif PlayerOneFormation == 3:
            self.FormationCornerKick(2)
        elif PlayerOneFormation == 4:
            self.FormationDagger(2)
        while not GameOver:
            self.__DisplayState()
            SquareIsValid = False
            Choice = 0
    ...
    def FormationTrench(self, CurrentPlayer):
        PlayerOnePieces = [1, 4, 8, 9]
        PlayerTwoPieces = [26, 27, 31, 34]
        if CurrentPlayer == 1:
            for piece in PlayerOnePieces:
                CurrentPiece = Piece("piece", self._Players[0], 1, "!")
                self._Board[piece].SetPiece(CurrentPiece)
            ...
            for piece in PlayerTwoPieces:
                CurrentPiece = Piece("piece", self._Players[1], 1, '"')
                self._Board[piece].SetPiece(CurrentPiece)
            CurrentPiece = Piece("mirza", self._Players[0], 5, "1")
            self._Board[self.__GetIndexOfSquare(10 + self._NoOfColumns // 2)].SetPiece(CurrentPiece)
    
    def FormationPincer(self, CurrentPlayer):
        PlayerOnePieces = [8, 9, 12, 17]
        PlayerTwoPieces = [18, 23, 26, 27]
        if CurrentPlayer == 1:
            for piece in PlayerOnePieces:
                CurrentPiece = Piece("piece", self._Players[0], 1, "!")
                self._Board[piece].SetPiece(CurrentPiece)
    ...
    def FormationCornerKick(self, CurrentPlayer):
        PlayerOnePieces = [8, 9, 10, 17]
        PlayerTwoPieces = [23, 26, 27, 28]
        if CurrentPlayer == 1:
            for piece in PlayerOnePieces:
                CurrentPiece = Piece("piece", self._Players[0], 1, "!")
                self._Board[piece].SetPiece(CurrentPiece)
    ...
    def FormationDagger(self, CurrentPlayer):
        PlayerOnePieces = [3, 9, 14, 15]
        PlayerTwoPieces = [20, 21, 26, 32]
        if CurrentPlayer == 1:
            for piece in PlayerOnePieces:
                CurrentPiece = Piece("piece", self._Players[0], 1, "!")
                self._Board[piece].SetPiece(CurrentPiece)
