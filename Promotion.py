...
#Implementation meaning that if a normal piece reaches the back rank on the opposite side of the board, it is promoted to a mirza.
#Unlikely to come up as doesnt make much sense, but good exercise to work one.
#Created a new method CheckMoethod which checks if the piece is on its respective back rank and which player's turn it is.
#If hthe conditions are filled, then a new mirza piece object is created, then added to the board array in the index of the old piece.
#Actually does make a litle sense, as game ends if opponents mirza enters your kotla, so could make that easier without losing your own mirza(s)
...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            if MoveLegal:
                ...
                self.CheckPromotion(self._CurrentPlayer, FinishSquareReference)
    ...
    def CheckPromotion(self, CurrentPlayer, FinishSquare):
        if FinishSquare // 10 == self._NoOfRows and CurrentPlayer.SameAs(self._Players[0]):
            print("Player One Promotion")
            CurrentPiece = Piece("mirza", self._Players[0], 5, "1")
            self._Board[self.__GetIndexOfSquare(FinishSquare)].SetPiece(CurrentPiece)
        elif FinishSquare // 10 == 1 and CurrentPlayer.SameAs(self._Players[1]):
            CurrentPiece = Piece("mirza", self._Players[1], 5, "2")
            self._Board[self.__GetIndexOfSquare(FinishSquare)].SetPiece(CurrentPiece)
            print("Player Two Promotion")
        else:
            print("NO Promotion")
