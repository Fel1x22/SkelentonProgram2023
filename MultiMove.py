...
class Dastan:
    ...
    def __MultiMove(self):
        ...
        SquareIsValid = False
        while not SquareIsValid:
            FinishOneSquareReference = self.__GetSquareReference("to move to (Move One)")
            SquareIsValid = self.__CheckSquareIsValid(FinishOneSquareReference, False)
        MoveOneLegal = self._CurrentPlayer.CheckPlayerMove(MoveOne, StartSquareReference, FinishOneSquareReference)
        if MoveOneLegal:
            PointsForPieceCapture = self.__CalculatePieceCapturePoints(FinishOneSquareReference)
            SquareIsValid = False
            while not SquareIsValid:
                FinishTwoSquareReference = self.__GetSquareReference("to move to (Move Two)")
                SquareIsValid = self.__CheckSquareIsValid(FinishTwoSquareReference, False)
            MoveTwoLegal = self._CurrentPlayer.CheckPlayerMove(MoveOne, FinishOneSquareReference, FinishTwoSquareReference)
            if MoveTwoLegal:
                PointsForPieceCapture = self.__CalculatePieceCapturePoints(FinishTwoSquareReference)
                self.__UpdateBoard(StartSquareReference, FinishTwoSquareReference)
                self.__UpdatePlayerScore(PointsForPieceCapture)
                self._CurrentPlayer.ChangeScore(-25)
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3), 8 for multimove or 9 to take the offer: "))
                if Choice == 8:
                    self.__MultiMove()
                    break
                elif Choice == 9:
